...
exports.pagingMenuList=function(params) {
    // console.log(params);
    var sql='select a.*, b.catalog_name as param_catalog_name from common_param_info a left join common_param_catalog_info b on a.param_catalogid = b.catalog_id';
    var order = ' order by a.param_catalogid, a.param_id ';
    var where = ' where 1 = 1 ';
    var countSql = 'select count(1) as total FROM  common_param_info a';

    var whereParam = [];

    if(params.menu_name) {
        where += ' and a.param_name like ? ';
 ...
 * 保存参数
 */
exports.saveParam=function (mapEntity) {
    return mysql_utils.save("common_param_info",mapEntity);
};
/**
 * 保存参数类别
 */
exports.saveCatalog=function (mapEntity) {
    return mysql_utils.save("common_param_catalog_info",mapEntity);
};
/**
 * 通过id修改参数
 */
exports.updateMenuById=function(conditions,update){
    return mysql_utils.update("common_param_info",conditions,update);
};
/**
 * 通过id修改类别
 */
exports.updateCatalogById=function(conditions,update){
    return mysql_utils.update("common_param_catalog_info",conditions,update);
};
/**
 * 查找所有菜单
 */
exports.findAllMenuList=function(cb){
    var sql="select catalog_id,catalog_name from common_param_catalog_info";
...
exports.findParentName=function (catalog_id) {
    var sql="select catalog_name from common_param_catalog_info where catalog_id = ?";

    return mysql_utils.find(sql,catalog_id);
}

/**
 * 读取菜单及菜单权限表生成树形目录json
 */
exports.generateMenuTree=function(sys_id,cb){
    var sql="select * from common_param_catalog_info where catalog_sysid= ?";
    var where=[];
    where.push(sys_id);
    mysql_utils.find(sql,where).then(function (result) {
        var tree=[{name:"全部",id:0,sys_id:sys_id,spread:true}];
        ...
        while(treeLength>0&&maxLoop>0){
            for(let i in result){
                var treeNode={
                    name:result[i].catalog_name,
                    id:result[i].catalog_id,
                    catalog_name:result[i].catalog_name,
                    catalog_remark:result[i].catalog_remark,
                    catalog_sysid:result[i].catalog_sysid,
                    spread:true
                };
                generateTree(tree,result[i].catalog_pid,treeNode);
            }
            maxLoop=maxLoop-1;
...
exports.searchClickNode=function (map,params) {
    ...
    if(map.param_catalogid){
        console.log('系统id存在',map);
        where += ' and a.param_catalogid = ? and a.param_sysid = ?';
        whereParam.push(map.param_catalogid);
        whereParam.push(map.param_sysid);
    }else {
        console.log('系统id不存在',map);
        where += ' and a.param_sysid = ?';
        whereParam.push(map.param_sysid);
    }

    if(params.param_name) {
        where += ' and a.param_name like ?';
        whereParam.push('%' + params.param_name + '%');
    }
