...
$(function() {
	...
	cron.change(function () {
	    ...
	    $.ajax({
			//type: 'get',
			method: 'get',
	        url: 'plan_release_route/cron.do',
	        dataType: "json",
	        data: { "CronExpression": $("#cron").val() },
	        success: function (data) {
	            if (data.result && data.result.length > 0) {
	                var strHTML = "<ul>";
	                for (var i = 0; i < data.result.length - 7; i++) {
	                    strHTML += "<li id=\"cron_time_"+i+"\">" + data.result[i] + "</li>";
	                }
	                strHTML +="</ul>"
	                $("#runTime").html(strHTML);
	            } else {
	                $("#runTime").html("<p id=\"formatError\" style=\"color:red\">格式错误</p>");
	            }
	        }
	    });
	});
